...
{
	static float cubeRotationSpeed = 2.0f;
	ImGui::SliderFloat("Cube Rotation Speed", &cubeRotationSpeed, 5, 20);

	static float cubeSpeed = 10.0f;
	ImGui::SliderFloat("Cube Speed", &cubeSpeed, 5, 20);

	if (Keyboard::Get()->Press(VK_SHIFT))
	{
		if (Keyboard::Get()->Press(VK_RIGHT))
			rotation.x += cubeRotationSpeed * Time::Delta();
		if (Keyboard::Get()->Press(VK_LEFT))
			rotation.x -= cubeRotationSpeed * Time::Delta();

		if (Keyboard::Get()->Press(VK_UP))
			rotation.z += cubeRotationSpeed * Time::Delta();
	...
	{
		if (Keyboard::Get()->Press(VK_RIGHT))
			position.x += cubeSpeed * Time::Delta();
		if (Keyboard::Get()->Press(VK_LEFT))
			position.x -= cubeSpeed * Time::Delta();

		if (Keyboard::Get()->Press(VK_UP))
			position.z += cubeSpeed * Time::Delta();
		if (Keyboard::Get()->Press(VK_DOWN))
			position.z -= cubeSpeed * Time::Delta();
	}
