...
/*
  ...
  답 : 4
    ...
    3 ) 정적 지역변수와 지역 변수는 한번 할당이 이루어 진다면 프로그램이
        종료될 때까지 남아있는 데이터다.
    4 ) 정적 지역변수와 지역 변수는 static 키워드를 이용하여 선언이 가능하다.
    5 ) 정적 지역변수와 지역 변수는 매개변수의 타입으로 사용할 수 가 없다.
      답 : 5
    ...
    4 ) 매개변수의 값을 변경하면 외부의 변수의 값이 변경될 수 있다.
    5 ) 해당 없음
      답 : 4
